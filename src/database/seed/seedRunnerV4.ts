...
const ComicSeedSchema = z.object({
  ...
  genres: z
    .array(
      z.union([z.object({ name: z.string() }), z.string().transform((val) => ({ name: val }))])
    )
    .optional(),
  type: z
...
class MetadataCache {
  ...
  async getOrCreateAuthor(name: string): Promise<number | null> {
    ...
    try {
      const [result] = await db.insert(author).values({ name }).onConflictDoNothing().returning();
  ...
  async getOrCreateArtist(name: string): Promise<number | null> {
    ...
    try {
      const [result] = await db.insert(artist).values({ name }).onConflictDoNothing().returning();
  ...
  async getOrCreateGenre(name: string): Promise<number | null> {
    ...
    try {
      const [result] = await db.insert(genre).values({ name }).onConflictDoNothing().returning();
...
/**
 * Legacy log function - wraps logger for compatibility
 * @param message - Message to log
 * @param force - Force logging even in quiet mode
 */
function log(message: string, force = false) {
  if (!logger) {
    // Fallback during initialization
    if (ARGS.VERBOSE || force) console.log(message);
    return;
  }

  // Route to logger based on message type
  if (message.includes("✅")) {
    logger.success(message.replace("✅ ", ""));
...
async function seedUsers() {
  ...
  for (const userData of usersData) {
    try {
      ...
      success++;
      logger.debug(`✓ ${validated.email}`);
    } catch (error) {
      errors++;
      const errorMsg = error instanceof Error ? error.message : String(error);
      errorList.push(`${userData.email}: ${errorMsg}`);
      logger.debug(`✗ ${userData.email}`);
    }
  }

  logger.success(`Users: ${success} succeeded, ${errors} failed`);
  if (errorList.length > 0 && ARGS.VERBOSE) {
    logger.warn(`Errors: ${errorList.join(", ")}`);
  }
}
...
async function seedComics(cache: MetadataCache) {
  ...
  for (const comicData of comicsData) {
    try {
      ...
      const authorId = validated.author?.name
        ? await cache.getOrCreateAuthor(validated.author.name)
        : null;
      const artistId = validated.artist?.name
        ? await cache.getOrCreateArtist(validated.artist.name)
        : null;
...
async function seedChapters() {
  ...
  for (const chapterData of chaptersData) {
    try {
      ...
      const chapterTitle =
        validated.title || validated.name || `Chapter ${chapterData.name || "Unknown"}`;
      ...
      if (!comicId) {
        const [foundComic] = await db
          .select()
          .from(comic)
          .where(eq(comic.slug, validated.comic.slug))
          .limit(1);
        if (!foundComic) {
          errors++;
      ...
      if (insertedChapter && validated.images) {
        const imageValues = validated.images.slice(0, 5).map((img, idx) => ({
          ...
          pageNumber: img.pageNumber
            ? typeof img.pageNumber === "string"
              ? parseInt(img.pageNumber, 10)
              : img.pageNumber
            : idx,
        }));
...
async function main() {
  ...
  try {
    ...
    // Report seed statistics if logger is available
    if (logger) {
