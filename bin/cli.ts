...
program.name("comicwise").description("ComicWise Platform Management CLI").version("1.0.0");
...
docker
  ...
  .action(async (options) => {
    ...
    try {
      const cmd = options.build ? "docker-compose up --build -d" : "docker-compose up -d";
      execSync(cmd, { stdio: "inherit" });
...
docker
  ...
  .action(async (options) => {
    ...
    try {
      const cmd = options.volumes ? "docker-compose down -v" : "docker-compose down";
      execSync(cmd, { stdio: "inherit" });
...
const maintain = program.command("maintain").description("Maintenance commands");
...
// Generate Commands
const generate = program.command("generate").alias("g").description("Code generation commands");
